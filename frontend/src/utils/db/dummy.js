export const POSTS = [{
        _id: "1",
        text: "Finally saw the seoul tower live! DREAM COME TRUE!!!",
        img: "/posts/post1.jpg",
        user: {
            username: "nathanaela_2029",
            profileImg: "/avatars/boy1.png",
            fullName: "natty B",
        },
        comments: [{
            _id: "1",
            text: "CUTIE!",
            user: {
                username: "janedoe",
                profileImg: "/avatars/girl1.jpg",
                fullName: "Jane Doe",
            },
        }, ],
        likes: ["6658s891", "6658s892", "6658s893", "6658s894"],
    },
    {
        _id: "2",
        text: "From the Screen to the Ring to the Pen to the KING!!!!",
        user: {
            username: "t.jiang2004",
            profileImg: "/avatars/boy2.png",
            fullName: "Terence Jiang",
        },
        comments: [{
            _id: "1",
            text: "Nice Tutorial",
            user: {
                username: "janedoe",
                profileImg: "/avatars/girl2.jpg",
                fullName: "Jane Doe",
            },
        }, ],
        likes: ["6658s891", "6658s892", "6658s893", "6658s894"],
    },
    {
        _id: "3",
        text: "With my army boys :D",
        img: "/posts/post2.jpg",
        user: {
            username: "johndoe",
            profileImg: "/avatars/boy3.png",
            fullName: "John Doe",
        },
        comments: [],
        likes: ["6658s891", "6658s892", "6658s893", "6658s894", "6658s895", "6658s896"],
    },
    {
        _id: "4",
        text: "What's cooking good looking",
        img: "/posts/post3.jpg",
        user: {
            username: "johndoe",
            profileImg: "/avatars/boy3.png",
            fullName: "John Doe",
        },
        comments: [{
            _id: "1",
            text: "Nice Tutorial",
            user: {
                username: "janedoe",
                profileImg: "/avatars/girl3.jpg",
                fullName: "Jane Doe",
            },
        }, ],
        likes: [
            "6658s891",
            "6658s892",
            "6658s893",
            "6658s894",
            "6658s895",
            "6658s896",
            "6658s897",
            "6658s898",
            "6658s899",
        ],
    },
];

export const USERS_FOR_RIGHT_PANEL = [{
        _id: "1",
        fullName: "Terence Jiang",
        username: "t.jiang2002",
        profileImg: "/avatars/boy2.png",
    },
    {
        _id: "2",
        fullName: "Jane Doe",
        username: "janedoe",
        profileImg: "/avatars/girl1.jpg",
    },
    {
        _id: "3",
        fullName: "Bob Doe",
        username: "bobdoe",
        profileImg: "/avatars/boy3.png",
    },
    {
        _id: "4",
        fullName: "Daisy Doe",
        username: "daisydoe",
        profileImg: "/avatars/girl2.jpg",
    },
];